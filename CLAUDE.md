...
- **Anthropic Claude** — Messages API + web search tool. Two request modes selected by the `sync` option (`--sync/--no-sync` on `gm:ai` and `process`, **sync is the CLI default**): synchronous streamed per-file `messages.stream().finalMessage()` calls ([`ClaudeAgent.runSync`](src/ai/claude-agent.ts), bounded by `SYNC_CONCURRENCY`, faster, full-rate) or the async Messages Batches API (`runBatch`, ~50% cheaper, delayed). `sync` defaults to `false` inside `ClaudeAgent`/`MetadataGenerator` (batch) so library/spike callers are unaffected; the two commands opt in. Both paths share `buildBatchParams` + `finalizeRawResults` (validation, post-process, failure-dir save) and feed the same retry loop. Cost telemetry drops the batch discount for sync via `computeCost(model, usage, {batch})`. Per-media model tiering: comics + other on `claude-opus-4-6`, audio + video on `claude-sonnet-4-6` (see [src/ai/pipelines/](src/ai/pipelines/)). Requires `ANTHROPIC_API_KEY` for the `gm:ai` command and the metadata stage of `process`.
