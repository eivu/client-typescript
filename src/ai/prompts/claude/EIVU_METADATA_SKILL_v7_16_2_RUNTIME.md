...
| 7 | 🔵 | ai:rating >= 4.5 → Masterwork tag required | rating without tag | + `tag: Eivu's AI Masterwork Collection` |
...
| 19 | 🔵 | `ai:skill_version` required on every file | omit | `ai:skill_version: 7.16.2` |
...
4.5 = Widespread critical acclaim OR major award recognition        → + Masterwork tag
...
- [ ] `ai:skill_version: 7.16.2` present · `ai:engine` present and set to THIS agent's own model name
...
- [ ] ai:rating >= 4.5 → `tag: Eivu's AI Masterwork Collection` after ai:engine
